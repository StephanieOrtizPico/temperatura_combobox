...
def convertir():
    ...
    cent = int(c.get())
    if combobox_kf.get()=="Kelvin":
        k = cent + 273.15
        t_resultados.insert(INSERT, f"\n{int(c.get())} °C equivalen a {k} °K")
    elif combobox_kf.get() == "Fahrenheit":
        f = cent*9/5 + 32
...
lb_c.config(bg="white", fg="thistle", font=("Helvetica", 18))
lb_c.place(x=240, y=60)

# caja de texto para valor en centigrados
entry_c = Entry(frame_entrada, textvariable=c)
entry_c.config(bg="white", fg="thistle", font=("Times New Roman", 18), width=6)
entry_c.focus_set()
entry_c.place(x=300,y=60)

# Combobox para kelvin y fahrenheit
def selection_changed(event):
    selection = combobox_kf.get()
combobox_kf = ttk.Combobox(values=["Fahrenheit", "Kelvin"])
combobox_kf.bind("<<ComboboxSelected>>", selection_changed)
combobox_kf.place(x=260, y=120)
...
t_resultados.config(bg="lavenderblush", fg="black", font=("Courier", 18))
